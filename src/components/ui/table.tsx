...
const TableRow = React.forwardRef<HTMLTableRowElement, React.HTMLAttributes<HTMLTableRowElement>>(
  ({ className, ...props }, ref) => (
    <tr
      ref={ref}
      className={cn(
        "hover:bg-background-subtle border-b transition-colors data-[state=selected]:bg-neutral-100",
        className
      )}
      {...props}
    />
...
const TableCell = React.forwardRef<HTMLTableCellElement, React.TdHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <td ref={ref} className={cn("h-[48px] px-4 align-middle [&:has([role=checkbox])]:pr-0", className)} {...props} />
  )
);
